...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 18 23:28:01 2021
...
ffdata = gff.famaFrench3Factor(frequency = 'm')
ffdata.rename(columns={"date_ff_factors": 'Date'}, inplace=True)
ffdata.set_index('Date', inplace=True)
for i in range(len(data)):
    ffdata = ffdata.merge(data[i],on='Date')

x = ffdata[['Mkt-RF', 'SMB', 'HML']]
A = ffdata['spy'] - ffdata['RF']
B = ffdata['ilmn'] - ffdata['RF']
C = ffdata['cmg'] - ffdata['RF']
D = ffdata['msci'] - ffdata['RF']
E = ffdata['rmd'] - ffdata['RF']
F = ffdata['ftnt'] - ffdata['RF']
G = ffdata['zbra'] - ffdata['RF']
H = ffdata['ter'] - ffdata['RF']
I = ffdata['rol'] - ffdata['RF']
J = ffdata['abmd'] - ffdata['RF']
K = ffdata['ua'] - ffdata['RF']
x = sm.add_constant(x)

for i in [A,B,C,D,E,F,G,H,I,J,K]:
    ffmodel = sm.OLS(i, x).fit()
    intercept, b1, b2, b3 = ffmodel.params
    rf = ffdata['RF'].mean()
    market_premium = ffdata['Mkt-RF'].mean()
    size_premium = ffdata['SMB'].mean()
    value_premium = ffdata['HML'].mean()
    expected_monthly_return = rf + b1 * market_premium + b2 * size_premium + b3 * value_premium 
    expected_yearly_return = expected_monthly_return * 12
    print("Expected yearly return: " + str(expected_yearly_return))

#backtest, determine if the factor is significant
#oilchange, unemployment rate, 
...
def set_params():
    ...
    g.refresh_rate = 30
    g.method = 'method'
    g.invest_by_group = Falseg.quantile = 0.1
    g.group = 1
    g.stocknum = 5
